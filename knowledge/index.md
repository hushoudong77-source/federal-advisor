...
> 最后整理：2026-06-04 10:15 | 文件数：90 | V5.8.2r28.2 在线 | 厄尔尼诺V3.1补充：棕榈油/橡胶强弹性+大豆/玉米边际确认 | 宏观危机状态机V1.0焊入 | 趋势突破交易系统V1.0入库

## 🔴 核心执行法典（唯一真源）

- [V5.8.2r25 全量合并版](analysis/v56/v57-merged.md) — ⭐当前上线真源。单文件参考入口。r25守东主权指令+回测驱动进攻重标定：仓位10%/ATR5.0×/C3放宽/30天冷却期/拆股过滤/CAGR 7.88%。spearhead+routing+macros三文件焊入。2026-05-21签发。
- [V5.8.2r24 §0 固定层宪法](analysis/v56/v56-constitution.md) — VEA+VTI 永不离场。r24版本号同步。2026-05-21签发。
- [V5.8.2r25 §2 路由判断](analysis/v56/v56-routing.md) — 全池21标路由逻辑。r25进攻候选池扩展至14标。2026-05-21签发。
- [V5.8.2r25 §3.1 进攻策略](analysis/v56/v56-spearhead.md) — Spearhead Protocol。r25全量重标定：仓位10%/ATR5.0×/C3放宽/30天冷却期/拆股过滤。2026-05-21签发。
...
- [V5.8.2r25 §1+§4+§5](analysis/v56/v56-macros.md) — 数据输入+波动率状态机+执行闭环。r25进攻审计门同步：仓位10%/ATR5.0×/单标上限20%。r24核心变更保留。2026-05-21签发。
- [V5.8.2r28 开火档案](fire-archive-v582.md) — 执行级快速参考。r28全量同步（2026-06-03），17标攻守全链路参数矩阵。含废墟收割M3固化参数。
...
- [A股数据三剑客对比矩阵 V1.0](concepts/a-stock-data-triad-v1.md) — Tushare/BaoStock/AKShare全维度实测对比。核心结论：Tushare是主力（付费，覆盖全量回测+宏观+美股），BaoStock是辅助（免费，分钟线/成分股/交易日历独有优势，但ETF仅2026年起），AKShare是兜底（免费但爬虫翻车率高）。2026-05-30入库。
